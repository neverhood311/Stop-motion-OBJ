...
class MeshSequenceSettings(bpy.types.PropertyGroup):
    ...
    frameMode = bpy.props.EnumProperty(
        items = [('0', 'Blank', 'Object disappears when frame is out of range'),
                ...
                ('2', 'Bounce', 'Play in reverse at the end of the frame range')],
        name='Frame Mode',
        default='1')
...
class MeshSequence:
    ...
    def setFrame(self, _frameNum):
        ...
        #2: Bounce
        if(frameMode == 0):
            if(idx < 1 or idx >= len(self.meshes)):
                idx = 0
        elif(frameMode == 1):
            if(idx < 1):
                idx = 1
            elif(idx >= len(self.meshes)):
                idx = len(self.meshes) - 1
        elif(frameMode == 2):
            #if idx / 
            idx = 1
        #print('index: ' + str(idx))
...
class LoadMeshSequence(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        MSO.loadSequenceFromFile(dirPath, fileName)
